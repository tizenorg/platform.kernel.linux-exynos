				 ...
				 struct drm_exynos_ipp_cmd_node *c_node)
{
	...
	for (i = 0; i < num_plane; ++i) {
		...
		if (m_node->buf_info.handles[i]) {
			size = exynos_drm_gem_get_size(drm_dev,
					...
					c_node->filp);
			if (plane_size > size) {
				DRM_ERROR("%dth buffers is smaller than the \
					required size of the plane\n", i);
				return -EINVAL;
			}
	...
	if (buf_size < img_size) {
		DRM_ERROR("Total size of buffers is smaller than the sum of \
			required size of each plane\n");
		return -EINVAL;
	}
