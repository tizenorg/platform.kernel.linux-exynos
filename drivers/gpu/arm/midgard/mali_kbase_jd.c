...
static void kbase_jd_resource_waiters_add(struct kbase_jd_atom *katom)
{
...
static void kbase_jd_resource_waiters_remove(struct kbase_jd_atom *katom)
{
	KBASE_DEBUG_ASSERT(katom);
	list_del(&katom->node);
}
...
static void kbase_cancel_resource_wait_job(struct kbase_jd_atom *katom)
{
	KBASE_DEBUG_ASSERT(katom);
...
static mali_error kbase_jd_pre_external_resources(struct kbase_jd_atom *katom, const struct base_jd_atom_v2 *user_atom)
{
	...
	unsigned long *excl_resvs_bitmap = NULL;
#endif
...
void kbase_jd_zap_context(struct kbase_context *kctx)
{
	 ...
	 * this prevents items being removed when calling job_done_nolock in kbase_cancel_resource_wait_job.
	 */
