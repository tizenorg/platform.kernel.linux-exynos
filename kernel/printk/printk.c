...
/*
 * kmsg dict format: "_PID=<pid>\0_TID=<tid>\0_COMM=<comm>"
 */
#define KMSG_DICT_MAX_LEN	(5 + 20 + 1 + 5 + 20 + 1 + 6 + TASK_COMM_LEN)
...
int kmsg_sys_buffer_add(size_t size, umode_t mode)
{
	...
	log_b_new = kmalloc(sizeof(struct log_buffer), GFP_KERNEL);
	if (!log_b_new)
		return -ENOMEM;
	...
	log_b_new->first_seq = 0;
	log_b_new->first_idx = 0;
	log_b_new->next_seq = 0;
	log_b_new->next_idx = 0;
	log_b_new->clear_seq = 0;
	log_b_new->clear_idx = 0;
	log_b_new->mode = mode;
